...
/* FONCTIONS                                                                  */
/******************************************************************************/
int check_mur (Point p1,Point p2,char tab[LIG][COL],char direction)
	{
	int x,y;
	if (direction == 'Z'){
		x = 0; y = -10;}
	...
	while (touche != SDLK_ESCAPE) // Ferme la fenêtre dès lors que la touche échap est enfoncée
		...
		if (tour == 1) // Au tour du joueur 1
			...
			if (touche == SDLK_UP || touche == SDLK_DOWN || touche == SDLK_LEFT || touche == SDLK_RIGHT)
				{
				dessiner_rectangle(p1 , 10, 10, blue) ;
				if (touche == SDLK_UP)
					direction = 'Z';
				if (touche == SDLK_DOWN)
					direction = 'S';
				...
				if (touche == SDLK_RIGHT)
					direction = 'D';
				if (check_mur(p1,p2,tab,direction) != 1)
					{
					if (check_mur(p1,p2,tab,direction) == p1.x+10 || check_mur(p1,p2,tab,direction) == p1.x-10)
						p1.x = check_mur(p1,p2,tab,direction);
					else
						p1.y = check_mur(p1,p2,tab,direction);
					}
